...
TGrid_Utility::TGrid_Utility(int Nescalas, int dimension, char* mapa )
{
    int i;
    char contador;
		contador = 0;
    ...
    m_GridDefinition.m_wireframe_mode = 0;
    m_GridDefinition.m_escalas = Nescalas;
    ...
    Inicial->m_GridDefinition = m_GridDefinition;
    ...
    for(i = 0; i <(Nescalas-1); i++)
       {
        ...
        Actual->Hijo->m_GridDefinition = m_GridDefinition;
...
TGrid_Utility::ActualizarGrillas(int rapidez,TCAMARA3D *CAM)
{
  ...
  CAM->Oz = CAM->Oz + SUMZ;
  if(CAM->Oz<100000.0) CAM->Oz = 100000.0;
  ...
  while(Actual!=NULL)
  {
   ...
   Actual->m_GridDefinition = m_GridDefinition;
   Actual->x+= SUMX;
   Actual->y+= SUMY;
   Actual = Actual->Hijo;
  }
...
/**************************************************************************/
  ...
  while( Actual != NULL )
  {
    ...
    if(Actual->PosRealCentroY<0)
        Actual->PosRealCentroY+=Mapa->MapaAlto*Mapa->DistanciaEntrePuntos;

    if(Actual->PosRealCentroX<0)
      Actual->PosRealCentroX+=Mapa->MapaAncho*Mapa->DistanciaEntrePuntos;
  ...
  if( DistanciaAlRelieve < m_GridDefinition.m_minimum_surface_altitude) {
        CAM->Oz =(float) Final->GP[Final->Tamanio/2].alt + m_GridDefinition.m_minimum_surface_altitude;
  }
...
{
  if(!SafeMode)
    Actual->ProyectarTodo( Inicial, Mapa, CAM->Oz, CAM);
  else
    Actual->ProyectarTodo_SafeMode( Inicial, Mapa, CAM->Oz, CAM );
  ...
  */
  ...
  while(Actual!=Final)
  {
    if(DistanciaAlRelieve <(Actual->rad * 400))
    {
      ...
      /** SOLO DIBUJA la grilla de esta escala si... esta a cierta distancia */
      if(DistanciaAlRelieve <(Actual->Hijo->rad * 400))
      {
        Actual->EnRango = 1;
        //Actual->m_GridDefinition = m_GridDefinition;
  ...
  if(DistanciaAlRelieve <(Final->rad * 400))
  {
    ...
    //Final->m_GridDefinition = m_GridDefinition;
    Final->EnRango = 1;
...
TGrid_Utility::Posicionar(int map_x,int map_y, float altitud, TCAMARA3D *CAM)
{
  if (Mapa && CAM && Inicial) {
    ...
    inivectors(0.0,0.0,altitud,CAM);
    ///Inicial->dvspp*Inicial->radm2
    ActualizarGrillas( Inicial->dvspp*Inicial->radm2 * 0.1, CAM );
