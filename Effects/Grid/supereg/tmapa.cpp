...
TMapa::CargarMapa(void)
{
    ...
    miarchivo = fopen("mapa\\mapax.mp", "rb");
...
/// el radio del cilindro del toro...
/// P = ancho del mapa (en unidades de distancia)
/// P = MapaAncho*DistanciaEntrePuntos
/// R = radiust = P / 2*PI = R
/// FR = factor de reduccion
  float FR = 3.1415f;
 radiust = (float)MapaAncho*(float)DistanciaEntrePuntos / (float) (2.0f*MO_PI*FR);
