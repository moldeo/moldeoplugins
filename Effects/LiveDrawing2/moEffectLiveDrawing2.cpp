...
void moEffectLiveDrawing2::Interaction( moIODeviceManager *IODeviceManager ) {
	...
	if (1==2)
	{
		if (leftClicked)
		{
			// Cuando no hay una tableta disponible, la "presi�n" queda determinada por la velocidad del mouse.
			float dx = penX - penX0;
			float dy = penY - penY0;
			...
			// La presi�n est� definida como la rec�proca de la velocidad: cuanto m�s r�pido
			// se mueva el mouse, m�s delgada ser� la l�nea.
			float pressure1;
			if (0 < diff) pressure1 = 1.0 / diff;
			else pressure1 = 1.0;

			// Utilizando la ecuaci�n de suavizado para que los cambio en presi�n sean cont�nuos.
